...
while true
   ...
   % ********************************************************************* 
if (j+1) <= nrows || (j-1) >= 1 || (i+1) <= ncols || (i-1) >= 1 ||  (j) == nrows || (j) == 1 || (i) == ncols || (i) == 1
    if (j+1) <= nrows
    ...
    if (j) == nrows
        up = sub2ind(size(map), i, j);
    end
    if (j) == 1
        down = sub2ind(size(map), i, j);
    end
    if (i) == ncols
        right = sub2ind(size(map), i, j);
    end
    if (i)== 1
        left = sub2ind(size(map), i, j);
    end
